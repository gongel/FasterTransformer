...
namespace fastertransformer {
    ...
    public:
        ...
        std::cout<<"batch_size_"<<batch_size<<
        std::endl;
        std::cout<<"max_seq_len_"<<seq_len<<
        std::endl;
        std::cout<<"head_num_"<<head_num<<
        std::endl;
        std::cout<<"size_per_head_"<<size_per_head<<
        std::endl;
        std::cout<<"memory_hidden_units_"<<memory_hidden_units<<
        std::endl;
        {
#ifndef NDEBUG
            PRINT_FUNC_NAME_();
            ...
            if (fd == NULL) {
                ...
                err = fscanf(fd, "%*d %*f %d %f %d %*f %d %*f %d %*f %d %f", &cublasAlgo_[0], &split_time,
                             &cublasAlgo_[1],
